...
- (void)reshapeLabel {
	self.label.font = [UIFont fontWithName:[self fontNameFromStyle:self.labelFontStyle] size:self.labelFontSize * self.gui.scaleHeight];
	[self.label sizeToFit];
	self.label.frame = CGRectMake(
		round(self.originalLabelPos.x * self.gui.scaleX),
		round((self.originalLabelPos.y * self.gui.scaleHeight) - (self.labelFontSize * 0.5 * self.gui.scaleHeight)),
