...
- (id)initWithAtomLine:(NSArray *)line andGui:(Gui *)gui {
	if(line.count < 23) { // sanity check
		LogWarn(@"Number2: cannot create, atom line length < 23");
		return nil;
	}
	...
	if(self) {
		...
		if(![self hasValidSendName] && ![self hasValidReceiveName]) {
			...
			LogVerbose(@"Numberbox2: dropping, send/receive names are empty");
			return nil;
		}
		...
		self.fillColor = [IEMWidget colorFromAtomColor:line[18]];
		self.controlColor = [IEMWidget colorFromAtomColor:line[19]];
		self.label.textColor = [IEMWidget colorFromAtomColor:line[20]];
...
- (void)reshape {
	...
	self.valueLabel.preferredMaxLayoutWidth =
		(ceil(charSize.width) * self.valueWidth) +
